...
class ContractTerm(models.Model):
    ...
    provider = fields.Many2one(
        'rea.agent',
        string="Provider",
        help="The provider of the commitment generated by the expression (or "
             "leave empty to use the same provider as the source commitment)")
    receiver = fields.Many2one(
        'rea.agent',
        string="Receiver",
        help="The receiver of the commitment generated by the expression (or )"
             "leave empty to use the same receiver as the source commitment)")
    condition_resource_type = fields.Many2one(
        'rea.resource.type',
        string='Resource type',
        help="This resource type must be used in the source commitment.")
    condition_commitment_type = fields.Many2one(
        'rea.commitment.type',
        string='Commitment type',
        help="The source commitment must be of this type.")
    ...
    def execute(self, commitment):
        for t in self:
            ...
            commitments = contract_function(
                ...
                t.provider.id or commitment.provider.id,
                t.receiver.id or commitment.receiver.id)
